/*
  User: Cloudy
  Date: 01/02/2022
  Time: 02:19
*/

package cz.cloudy.minecraft.toxicmc.components.economics;

/**
 * @author Cloudy
 */
public class PriceConst {
    public static final int[][] COMPANY_BANNER = new int[][] {
            new int[] {1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000},
            new int[] {2000, 4000, 6000, 8000, 10000, 12000, 14000, 16000},
            new int[] {3000, 6000, 9000, 12000, 15000, 18000, 21000, 24000},
            new int[] {4000, 8000, 12000, 16000, 20000, 24000, 28000, 32000},
            new int[] {5000, 10000, 15000, 20000},
            new int[] {6000, 12000, 18000, 24000},
            new int[] {7000, 14000, 21000, 28000},
            new int[] {8000, 16000, 24000, 32000}
    };

    public static final int AREA_BLOCK = 5;
    public static final float AREA_BLOCK_EXPENSE = .5f;
}
